...
bool hash_table_insert(HashTable *ht, int key) {
  ...
  if (linked_list_exists(&ht->lists[bkt], key)) {
    Pthread_mutex_unlock(&ht->lock);
    return false;
  }
  linked_list_add(&ht->lists[bkt], key);
  Pthread_mutex_unlock(&ht->lock);
  return true;
}
...
bool hash_table_exists(HashTable *ht, int key) {
  Pthread_mutex_lock(&ht->lock);
  size_t bkt = hash_key(key, ht->buckets);
  bool exists = linked_list_exists(&ht->lists[bkt], key);
  Pthread_mutex_unlock(&ht->lock);
  return exists;
}
...
bool hash_table_remove(HashTable *ht, int key) {
  Pthread_mutex_lock(&ht->lock);
  size_t bkt = hash_key(key, ht->buckets);
  bool removed = linked_list_remove(&ht->lists[bkt], key);
  Pthread_mutex_unlock(&ht->lock);
  return removed;
}
